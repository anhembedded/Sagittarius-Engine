...
### High (Sprint)
3. **kernel/extension_manager.py** - Extension lifecycle failures
4. **runtime/tasks/task_manager.py** - Task execution and shutdown failures
 
### Medium (Backlog)
5. **runtime/hosted/hosted_service_manager.py** - Service lifecycle
6. **runtime/scheduler/scheduler.py** - Job execution failures
7. **runtime/async_runtime/async_runtime.py** - Async loop edge cases
8. **infrastructure/container/std_container.py** - Resolution failures
 
### Low (Optional)
9. **infrastructure/event_bus/** - Already has decent coverage
