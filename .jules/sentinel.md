...
**Prevention:** Require a `base_path` parameter with a safe default (or explicitly pass it from callers), resolve all paths with `os.path.realpath`, and use `os.path.commonpath` to ensure the final path resides within the base directory. If an attempt to escape is detected, raise `PathTraversalError`.
## 2024-05-18 - [Fail-Open Path Traversal in BatchInputPort]
**Vulnerability:** BatchInputPort's `base_path` parameter defaulted to `None` and entirely skipped the `PathTraversalError` check if `base_path` was not explicitly provided by the caller, allowing arbitrary absolute paths to be read (fail-open model).
**Learning:** Security controls like path confinement checks must always be enforced unconditionally. Relying on an optional argument (e.g. `None`) to conditionally bypass a security check creates a dangerous fail-open scenario where developers accidentally leave the system insecure by omitting the argument.
**Prevention:** Always default base paths to an empty string `""` (representing the current working directory) or a secure default, and apply traversal checks unconditionally so that failure is safe (fail-closed model).
