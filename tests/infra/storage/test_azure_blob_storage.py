...
class TestAzureBlobStorage:
    def test_azure_blob_storage__read__success(self):
        with patch("src.infra.storage.azure_blob_storage.BlobServiceClient") as mock_blob_service_client:
            ...
            mock_blob_client.download_blob.return_value = MagicMock(readall=MagicMock(return_value=b"test data"))

            storage = AzureBlobStorage(connection_string="dummy_conn_str", container_name="my-container")
            data = storage.read("path/to/file.txt")

            assert data == b"test data"
            mock_blob_service_client.from_connection_string.assert_called_once_with("dummy_conn_str")
            mock_service_client.get_container_client.assert_called_once_with("my-container")
            mock_container_client.get_blob_client.assert_called_once_with("path/to/file.txt")
    ...
    def test_azure_blob_storage__init__missing_azure_storage_blob(self):
        try:
            with patch.dict("sys.modules", {"azure.storage.blob": None}):
                ...
                with pytest.raises(ImportError, match="azure-storage-blob is not installed"):
                    azure_module.AzureBlobStorage(connection_string="dummy", container_name="dummy")
        finally:
            importlib.reload(azure_module)
