...
def test_base_repository__add__integrity_error_bubbles_up():
    ...
    with unittest.mock.patch.dict(
        "sys.modules", {"sqlalchemy": mock_sqlalchemy, "sqlalchemy.exc": mock_exc}
    ):
        ...
        mock_session.add.side_effect = mock_sqlalchemy.exc.IntegrityError(
            "Unique constraint failed"
        )

        with pytest.raises(
            mock_sqlalchemy.exc.IntegrityError, match="Unique constraint failed"
        ):
            repo.add(entity)
