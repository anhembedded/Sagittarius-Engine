    ...
    def assert_emitted(self, event_name: str, times: int = 1):
        ...
        if count != times:
            raise AssertionError(
                f"Expected event '{event_name}' to be emitted {times} times, but was emitted {count} times."
            )
...
def assert_event_emitted(event_bus: IEventBus, event_name: str, times: int = 1):
    ...
    if hasattr(event_bus, "emitted_events"):
        ...
        raise ValueError(
            "The provided event_bus does not have tracking enabled. Use the 'event_bus' fixture or EventBusSpy."
        )
