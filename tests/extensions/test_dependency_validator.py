...
def test_dependency_validator_success(mock_find_spec, mock_context):
    ...
    mock_context.logger.info.assert_called_with("Pre-flight check passed. All critical dependencies found.")
...
def test_dependency_validator_failure(mock_exit, mock_find_spec, mock_context):
    ...
    extension = DependencyValidatorExtension(required_packages=["existing_pkg", "missing_pkg"])
