...
def generate_ui(port: int):
    ...
    table = Table(title="Background Tasks", box=box.ROUNDED, expand=True, border_style="blue")
    ...
    scheduler_table = Table(title="Scheduled Jobs", box=box.ROUNDED, expand=True, border_style="yellow")
    ...
    overview_text += f"[b]CPU:[/b] {env.get('cpu_percent')} | [b]RAM:[/b] {env.get('ram_mb')}\n\n"
    ...
    ext_panel = Panel(ext_text, title="Extensions & Pipeline", border_style="yellow", box=box.ROUNDED)
    ...
    cb_panel = Panel(cb_text, title="Event Bus & Config", border_style="magenta", box=box.ROUNDED)
...
def generate_config_ui(port: int):
    ...
    if not data:
        return Panel(f"[red]🔴 Connection Error[/red]\nCannot connect to Engine at `http://localhost:{port}/config`.", title="Error", border_style="red")
...
def generate_events_ui(port: int):
    ...
    if not data:
        return Panel(f"[red]🔴 Connection Error[/red]\nCannot connect to Engine.", title="Error", border_style="red")
    
    events = data.get("events", [])
    table = Table(title="Live Event Stream (Last 100)", box=box.ROUNDED, expand=True, border_style="magenta")
...
def generate_tasks_ui(port: int):
    ...
    tasks = data.get("tasks", [])
    table = Table(title="Task Manager", box=box.ROUNDED, expand=True, border_style="blue")
    ...
    for t in tasks:
        ...
        table.add_row(t.get("id"), t.get("name"), status_str, t.get("runtime"), error_msg)
...
def main():
    ...
    parser.add_argument("--view", type=str, default="summary", choices=["summary", "config", "events", "tasks"], help="Dashboard view to display")
