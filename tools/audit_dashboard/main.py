...
try:
    ...
    class StdLibContainer:
        def singleton(self, interface, instance): pass
        def resolve(self, cls): return cls()
    class MemoryEventBus:
        def on(self, event_name, handler): pass
        def emit(self, event_name, event): pass
    class App:
        def __init__(self, container, event_bus):
            self.container = container
            self.event_bus = event_bus
        def boot(self, auto_discover=None): pass
        def execute(self, cmd_cls, data=None): pass
...
def main():
    ...
    parser.add_argument('--ui-only', action='store_true', help="Run UI only without connecting to the engine")
