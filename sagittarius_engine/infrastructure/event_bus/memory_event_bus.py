...
class MemoryEventBus(IEventBus):
    ...
    def emit(self, event_name: str, data: Any = None) -> None:
        ...
        # Lock-free read: _handlers dict is thread-safe for reading, and the values are immutable tuples.
    ...
    def on(self, event_name: str, handler: Callable) -> None:
        ...
        with self._lock:
            ...
            if handler not in current_handlers:
                # Copy-on-write: assign a new tuple
    ...
    def off(self, event_name: str, handler: Callable) -> None:
        ...
        with self._lock:
            ...
            if handler in current_handlers:
                # Copy-on-write: assign a new tuple without the handler
                # Note: We keep the event_name key even if empty to satisfy tests
                self._handlers[event_name] = tuple(h for h in current_handlers if h != handler)
