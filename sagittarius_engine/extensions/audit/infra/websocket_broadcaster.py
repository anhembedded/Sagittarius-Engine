            ...
            try:
                if 'websockets.asyncio' in sys.modules:
                    async with serve(self._handler, self.host, self.port):
                        self._logger.info(f"Websocket Broadcaster (asyncio) listening on ws://{self.host}:{self.port}")
                        await self._stop_event.wait()
                else:
                    start_server = serve(self._handler, self.host, self.port)
                    self._server = await start_server
                    self._logger.info(f"Websocket Broadcaster (legacy) listening on ws://{self.host}:{self.port}")
                    await self._stop_event.wait()
