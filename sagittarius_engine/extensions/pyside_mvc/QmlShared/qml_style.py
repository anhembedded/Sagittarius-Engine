...
# SpinBox in an app's QML depends on those overrides, so the app-wide style
...
def ensure_qml_style() -> None:
    ...
    unit tests, which never run the app's own bootstrapper) get the same
    styling as the real app, instead of tests passing against native chrome
