...
def main():
    ...
    new_parser.add_argument("template", help="Template name (e.g. minimal, clean, ddd, mvc)")
    new_parser.add_argument("project_name", help="Name of the project to create")
    new_parser.add_argument("--output-dir", default=".", help="Directory to create the project in (default: .)")
    ...
    if args.command == "new":
        ...
        if args.template not in available_templates:
            print(f"Error: Template '{args.template}' not found. Available templates: {', '.join(available_templates)}")
            sys.exit(1)
        ...
        try:
            ...
            print(f"Project '{args.project_name}' created successfully from template '{args.template}' at '{project_path}'.")
